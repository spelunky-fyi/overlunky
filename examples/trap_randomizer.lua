...
ceiling_to = {46, 60}
floortypes = {
    ENT_TYPE.FLOOR_GENERIC, ENT_TYPE.FLOORSTYLED_TEMPLE, ENT_TYPE.FLOORSTYLED_COG, ENT_TYPE.FLOORSTYLED_BABYLON, ENT_TYPE.FLOORSTYLED_DUAT, ENT_TYPE.FLOORSTYLED_STONE,
    ENT_TYPE.FLOOR_ARROW_TRAP, ENT_TYPE.FLOOR_POISONED_ARROW_TRAP, ENT_TYPE.FLOOR_JUNGLE_SPEAR_TRAP, ENT_TYPE.FLOOR_LASER_TRAP, ENT_TYPE.FLOOR_SPARK_TRAP, ENT_TYPE.FLOOR_TIMED_FORCEFIELD, ENT_TYPE.FLOOR_SPIKEBALL_CEILING, ENT_TYPE.FLOOR_STICKYTRAP_CEILING
}
to = 0

function replace_trap(v)
    x, y, l = get_position(v)
    if l == LAYER.BACK and state.theme ~= THEME.ICE_CAVES then return false end
    ...
    if #shop ~= 0 or #shoppie ~= 0 or #tusk ~= 0 or #altar ~= 0 or #plr ~= 0 then return false end -- don't put traps around shops, entrance or altars
