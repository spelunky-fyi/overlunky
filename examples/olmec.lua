...
set_callback(function(draw_ctx)
    ...
    for i, olmec_uid in ipairs(olmecs) do
        ...
        draw_ctx:draw_text(x, y, 0, "Olmec attack phase: " .. attack_phase, white)
        y = y - 0.05
        draw_ctx:draw_text(x, y, 0, "Attack timer: " .. tostring(olmec.attack_timer), white)
        y = y - 0.05
        draw_ctx:draw_text(x, y, 0, "Jump timer: " .. tostring(olmec.jump_timer), white)
        y = y - 0.05
        draw_ctx:draw_text(x, y, 0, "AI timer: " .. tostring(olmec.ai_timer), white)
        y = y - 0.05
        
        move_direction = "down"
        if olmec.move_direction < 0 then move_direction = "left" 
        elseif olmec.move_direction > 0 then move_direction = "right" end
        draw_ctx:draw_text(x, y, 0, "Move direction: " .. move_direction, white)
        y = y - 0.05
        
        if olmec.attack_phase == 1 then
            draw_text(x, y, 0, "Bomb salvos left: " .. olmec.phase1_amount_of_bomb_salvos, white)
            y = y - 0.05
            draw_ctx:draw_text(x, y, 0, "Broken floaters: " .. olmec:broken_floaters(), white)
            y = y - 0.05
            draw_ctx:draw_text(x, y, 0, "Unknown attack state: " .. olmec.unknown_attack_state, white)
            y = y - 0.05
