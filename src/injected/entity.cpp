...
size_t entities_ptr() {
    ...
    return mem.at_exe(decode_pc(
            ...
            find_inst(
                    mem.exe(),
                    "\x48\xB8\x02\x55\xA7\x74\x52\x9D\x51\x43"s,
                    mem.after_bundle
            ) - 7
    ));
}

std::vector<EntityItem>
list_entities() {
...
size_t
to_id(size_t map_ptr, std::string name) {
    auto map = reinterpret_cast<EntityMap *>(map_ptr + NAME_TO_INDEX);
...
Carry get_carry() {
    ONCE(Carry) {
        ...
        auto off = find_inst(
                memory.exe(),
                "\xBA\xE1\x00\x00\x00\x49\x8B\xCD"s,
                memory.after_bundle);
...
std::pair<float, float> Entity::position_self() const { return std::pair<float, float>(x, y); }
