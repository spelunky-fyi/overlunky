...
size_t find_base(Process proc, std::string name)
{
    ...
    for (auto &&item : map)
    {
        if (name.find(item.name) != std::string::npos)
            return item.addr;
    }
    ...
    const std::string &function)
{
    ...
    {
        PANIC(
            "Cannot find the address of the library in current process: %s",
            library.data());
    }
    ...
    {
        PANIC(
            "Cannot find the address of the function in current process: %s :: %s",
            library.data(), function.data());
    }

    return reinterpret_cast<LPTHREAD_START_ROUTINE>(
        addr - library_ptr + find_base(proc, library));
}
