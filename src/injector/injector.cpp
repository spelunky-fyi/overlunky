...
std::vector<MemoryMap> memory_map(const Process &proc) {
    ...
    while (VirtualQueryEx(proc.handle, (LPVOID)cur, &mbf, sizeof(mbf)) != 0) {
        auto res = GetModuleBaseNameA(proc.handle, (HMODULE)cur, buffer,
                                      sizeof(buffer));
        if (res != 0) {
...
LPTHREAD_START_ROUTINE find_function(const Process &proc,
    ...
    return reinterpret_cast<LPTHREAD_START_ROUTINE>(addr - library_ptr +
                                                    find_base(proc, library));
}
...
#undef PROCESSENTRY32
    ...
    while (proc) {
        ...
        if (auto delim = strrchr(name, '\\')) name = delim;
