...
struct ThemeHooksInfo
{
    ...
    std::map<THEME_OVERRIDE, std::vector<HookWithId<bool(ThemeInfo*)>>> pre;
    std::map<THEME_OVERRIDE, std::vector<HookWithId<void(ThemeInfo*)>>> post;
    ...
    static void hook_void(ThemeInfo* self, THEME_OVERRIDE index)
    {
        hook_vtable_no_dtor<void(ThemeInfo*)>(
            self,
            [index](ThemeInfo* lmbd_self, void (*original)(ThemeInfo*))
            {
    ...
    template <class T, auto PreHooks, auto PostHooks>
    static void hook_non_void(ThemeInfo* self, THEME_OVERRIDE index)
    {
        hook_vtable_no_dtor<T(ThemeInfo*)>(
            self,
            [index](ThemeInfo* lmbd_self, T (*original)(ThemeInfo*))
            {
                ThemeHooksInfo& hook_info = lmbd_self->get_hooks();
    ...
    static void hook_bool(ThemeInfo* self, THEME_OVERRIDE index)
    {
        hook_non_void<bool, &ThemeHooksInfo::pre_bool, &ThemeHooksInfo::post>(self, index);
    }

    static void hook_u8(ThemeInfo* self, THEME_OVERRIDE index)
    {
        hook_non_void<uint8_t, &ThemeHooksInfo::pre_u8, &ThemeHooksInfo::post>(self, index);
    }

    static void hook_u32(ThemeInfo* self, THEME_OVERRIDE index)
    {
        hook_non_void<uint32_t, &ThemeHooksInfo::pre_u32, &ThemeHooksInfo::post>(self, index);
    }

    static void hook_float(ThemeInfo* self, THEME_OVERRIDE index)
    {
        hook_non_void<float, &ThemeHooksInfo::pre_float, &ThemeHooksInfo::post>(self, index);
    }

    static void hook_texture(ThemeInfo* self, THEME_OVERRIDE index)
    {
...
std::map<ThemeInfo*, ThemeHooksInfo> g_theme_hooks;

ThemeHooksInfo& ThemeInfo::get_hooks()
{
    return g_theme_hooks[this];
}
...
{
    for (auto& [theme, funcs] : g_theme_hooks[this].pre)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
    for (auto& [theme, funcs] : g_theme_hooks[this].post)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
    for (auto& [theme, funcs] : g_theme_hooks[this].pre_bool)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
    for (auto& [theme, funcs] : g_theme_hooks[this].pre_u8)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
    for (auto& [theme, funcs] : g_theme_hooks[this].pre_u32)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
    for (auto& [theme, funcs] : g_theme_hooks[this].pre_float)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
    for (auto& [theme, funcs] : g_theme_hooks[this].pre_texture)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
    for (auto& [theme, funcs] : g_theme_hooks[this].post_texture)
    {
        std::erase_if(funcs, [id](auto& hook)
                      { return hook.id == id; });
    }
...
void ThemeInfo::set_pre(std::uint32_t reserved_callback_id, THEME_OVERRIDE index, std::function<bool(ThemeInfo*)> pre_func)
{
    ThemeHooksInfo& hook_info = get_hooks();
    if (!get_hook_function((void***)this, (uint8_t)index))
        ThemeHooksInfo::hook_void(this, index);
    hook_info.pre[index].push_back({reserved_callback_id, std::move(pre_func)});
}

void ThemeInfo::set_post(std::uint32_t reserved_callback_id, THEME_OVERRIDE index, std::function<void(ThemeInfo*)> post_func)
{
    ThemeHooksInfo& hook_info = get_hooks();
    if (!get_hook_function((void***)this, (uint8_t)index))
        ThemeHooksInfo::hook_void(this, index);
    hook_info.post[index].push_back({reserved_callback_id, std::move(post_func)});
}
...
void ThemeInfo::set_pre_bool(std::uint32_t reserved_callback_id, THEME_OVERRIDE index, std::function<std::optional<bool>(ThemeInfo*)> pre_func)
{
    ThemeHooksInfo& hook_info = get_hooks();
    if (!get_hook_function((void***)this, (uint8_t)index))
        ThemeHooksInfo::hook_bool(this, index);
...
void ThemeInfo::set_pre_u8(std::uint32_t reserved_callback_id, THEME_OVERRIDE index, std::function<std::optional<uint8_t>(ThemeInfo*)> pre_func)
{
    ThemeHooksInfo& hook_info = get_hooks();
    if (!get_hook_function((void***)this, (uint8_t)index))
        ThemeHooksInfo::hook_u8(this, index);
...
void ThemeInfo::set_pre_u32(std::uint32_t reserved_callback_id, THEME_OVERRIDE index, std::function<std::optional<uint32_t>(ThemeInfo*)> pre_func)
{
    ThemeHooksInfo& hook_info = get_hooks();
    if (!get_hook_function((void***)this, (uint8_t)index))
        ThemeHooksInfo::hook_u32(this, index);
...
void ThemeInfo::set_pre_float(std::uint32_t reserved_callback_id, THEME_OVERRIDE index, std::function<std::optional<float>(ThemeInfo*)> pre_func)
{
    ThemeHooksInfo& hook_info = get_hooks();
    if (!get_hook_function((void***)this, (uint8_t)index))
        ThemeHooksInfo::hook_float(this, index);
...
void ThemeInfo::set_pre_texture(std::uint32_t reserved_callback_id, THEME_OVERRIDE index, std::function<std::optional<uint32_t>(ThemeInfo*, int32_t)> pre_func)
{
    ThemeHooksInfo& hook_info = get_hooks();
    if (!get_hook_function((void***)this, (uint8_t)index))
        ThemeHooksInfo::hook_texture(this, index);
...
void grow_poles(LAYER l, uint32_t max_lengh, AABB area, bool destroy_broken)
{
    ...
    if (l == LAYER::BOTH)
    {
        grow_poles(LAYER::BACK, max_lengh, area, destroy_broken);
    }
}
