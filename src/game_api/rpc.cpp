...
uint32_t spawn_entity(size_t id, float x = 0.0f, float y = 0.0f, bool s = false,
                      float vx = 0.0f, float vy = 0.0f, bool snap = false) {
...
void spawn_backdoor(float x, float y) {
    ...
    state.layer(0)->spawn_entity(37, x + _x, y + _y - 1.0, false, 0.0, 0.0,
                                 true);
    state.layer(1)->spawn_entity(37, x + _x, y + _y - 1.0, false, 0.0, 0.0,
                                 true);
...
void godmode(bool g) { State::get().godmode(g); }

void darkmode(bool g) { State::get().darkmode(g); }

void zoom(float level) { State::get().zoom(level); }

float get_zoom_level() { return State::get().get_zoom_level(); }
