...
struct EntityHooksInfo
{
    ...
    std::vector<HookWithId<bool(Entity*)>> pre_update;
    std::vector<HookWithId<void(Entity*)>> post_update;
...
class Entity
{
  public:
    ...
    void set_pre_update(std::uint32_t reserved_callback_id, std::function<bool(Entity*)> pre_update);
    void set_post_update(std::uint32_t reserved_callback_id, std::function<void(Entity*)> post_update);
